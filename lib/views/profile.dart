...
import 'package:graduation___part1/views/chat_cubit.dart' as cubit;
import 'dart:convert';
...
class ProfilePageState extends State<ProfilePage>
  ...
  Future<void> _initializeUserData() async {
    try {
      ...
      // Store real authentication data for the chat service
      // In a real app, this would be populated with a token from your authentication system
      await prefs.setString('CookieToken', 'token');

      // Use email as a unique identifier since 'id' property doesn't exist
      await prefs.setString('UserId', widget.profile.email ?? 'default-user');
      ...
      setState(() {
        _isInitializing = false;
        _error = "Failed to initialize chat. Please try again later.";
      });
    }
  }
  ...
  void _toggleChat() {
    if (_isInitializing) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Chat is still initializing, please wait..."),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      ...
                      label:
                          Text(_isInitializing ? "Loading..." : "Contact Me"),
                    ...
                    child: BlocProvider(
                      ...
                      child: ChatWindow(
                        ...
                        onReply: (message) {
                          ...
                          chatCubit.sendMessage("Reply to: ${message.text}",
...
class ProfilePageState extends State<ProfilePage>
  ...
  Future<void> _initializeUserData() async {
    final prefs = await SharedPreferences.getInstance();
    // Use the email as a unique identifier for this user
    await prefs.setString('UserId', widget.profile.email ?? 'default-user');

    // Note: We don't set CookieToken here as it should already be set by the login process
    // This ensures we're using the same token for all chat communications
  }
                    ...
                    child: BlocProvider(
                      ...
                      child: ChatWindow(
                        ...
                        onReply: (message) {
                          ...
                          // Show a reply dialog or directly prepare the UI for reply
                          chatCubit.sendMessage("Reply to: ${message.text}",
                              widget.profile.email ?? 'default-user');
                        },
                      ),
                    ),
                  ),
...
class ProfilePageState extends State<ProfilePage>
  ...
  void initState() {
    ...
    // Initialize mock user data in SharedPreferences
    _initializeUserData();
  }
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // Chat window with proper animation and Stack positioning
          if (_isChatVisible)
            Positioned(
              bottom: 0,
              left: 0,
  ...
  Widget _buildContent() {
    // ... keep existing code (the content building method)
    switch (selectedTab) {
      case 0:
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
