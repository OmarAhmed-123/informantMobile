...
class CreateAdViewState extends State<CreateAdView>
  ...
  // Update the _createAd method in the CreateAdViewState class
  ...
  Future<void> _createAd() async {
    if (formKey.currentState!.validate()) {
      try {
        ...
        // Convert media files to URLs (in a real app, you would upload these files to a server)
        List<String> imageUrls = mediaFiles.map((file) => file.path).toList();

        // Create new ad with the specified format
        List<Map<String, dynamic>> newAd = [
          {
            "name": nameController.text,
            "details": descriptionController.text,
            "stars": 0, // Initial rating
            "potentialRevenue": double.parse(priceController.text),
            "images": imageUrls,
            "availablePlaces": 10, // Default value, could be made configurable
            "creatorName": "COMPANY NAME" // Could be fetched from user profile
          }
        ];
...
import 'package:image_picker/image_picker.dart';
import 'package:provider/provider.dart';
import '../view_model/create_ad_view_model.dart';
import '../animations/fade_scale_animation.dart';
import 'widgets/media_preview.dart';
import 'widgets/form_fields.dart';
import 'widgets/submit_button.dart';
import '../../home/view/home_view.dart';
import '../../home/Bar_home/view/barOfHome.dart';
...
class CreateAdViewState extends State<CreateAdView>
    with TickerProviderStateMixin {
  late FadeScaleAnimation _animation;
  late AnimationController _formAnimationController;
  late Animation<Offset> _slideAnimation;
  ...
  void _initializeAnimations() {
    _animation = FadeScaleAnimation(vsync: this);
  ...
  Widget _buildBody() {
    return Consumer<CreateAdViewModel>(
                ...
                child: Column(
                  children: [
                    MediaPreview(
                      ...
                      videoControllers: viewModel.videoControllers,
                      fadeAnimation: _animation.fadeAnimation,
                      scaleAnimation: _animation.scaleAnimation,
                      onRemoveMedia: viewModel.removeMedia,
                      onAddMedia: () =>
                          _showMediaSourceDialog(context, viewModel),
                    ),
                    const SizedBox(height: 24),
                    FormFields(
  ...
  void _showMediaSourceDialog(
      BuildContext context, CreateAdViewModel viewModel) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              _showMediaTypeDialog(context, viewModel, ImageSource.gallery);
            },
          ...
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              _showMediaTypeDialog(context, viewModel, ImageSource.camera);
            },
  ...
  void _showMediaTypeDialog(
      BuildContext context, CreateAdViewModel viewModel, ImageSource source) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: Colors.grey[900],
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
        title: Text(
          source == ImageSource.camera ? 'Take Media' : 'Choose from Gallery',
          style: const TextStyle(color: Colors.white),
        ),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              viewModel.pickMedia(source, isVideo: false);
            },
          ...
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              viewModel.pickMedia(source, isVideo: true);
            },
  ...
  Future<void> _handleSubmit(
      BuildContext context, CreateAdViewModel viewModel) async {
    try {
      await viewModel.createAd();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Ad created successfully!'),
