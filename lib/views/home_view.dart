    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return Transform.translate(
          ...
          child: Container(
            height: 200, // Reduced size of the list view
            child: PageView.builder(
              controller: _pageController,
              ...
              itemBuilder: (context, index) {
                ...
                return AnimatedOpacity(
                  duration: const Duration(
                      milliseconds: 3000), // Duration for appearing
...
class _HomeViewState extends State<HomeView> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _verticalSlideAnimation = Tween<double>(begin: 100, end: 0).animate(
      CurvedAnimation(
        parent: _verticalListAnimationController,
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return Transform.translate(
          offset: Offset(0, _verticalSlideAnimation.value),
          child: Container(
            // Increased height and width to display all content
            height: MediaQuery.of(context).size.height * 0.8,
            width: MediaQuery.of(context).size.width * 0.9,
            child: ListView.builder(
              physics:
                  const BouncingScrollPhysics(), // Allows scrolling when needed
              itemCount: filteredAds.length,
              itemBuilder: (context, index) {
                final ad = filteredAds[index];
                final isCurrentAd = index == _currentVerticalPage;

                return AnimatedOpacity(
                  duration: const Duration(milliseconds: 1000),
                  opacity: isCurrentAd && _isAdVisible ? 1.0 : 0.0,
                  child: AnimatedContainer(
                    duration: const Duration(milliseconds: 500),
                    curve: Curves.easeOutCubic,
                    margin: EdgeInsets.symmetric(
                      horizontal: 16,
                      vertical: isCurrentAd ? 8 : 0,
                    ),
                    height: isCurrentAd ? 250 : 0,
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        begin: Alignment.topLeft,
                        end: Alignment.bottomRight,
