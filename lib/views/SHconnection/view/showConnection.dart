/*
//not include the adding animation
...
import '../animations/connection_animations.dart';
import '../view_model/connection_view_model.dart';
import 'widgets/app_header.dart';
import 'widgets/internet_icon.dart';
...
class ShowConnectionState extends State<ShowConnection>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
  late ConnectionAnimations _animations;
  ...
  void initState() {
    ...
    _animations = ConnectionAnimations(controller: _controller);
    _viewModel = ConnectionViewModel(animationController: _controller);
    _controller.forward();
  }
      ...
      child: Scaffold(
        body: Container(
          ...
          child: Column(
            ...
            children: [
              const SizedBox(height: 80),
              AppHeader(fadeAnimation: _animations.fadeIn),
              const Spacer(),
              Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    InternetIcon(
                      scaleAnimation: _animations.scale,
                      rotateAnimation: _animations.rotate,
                    ),
                    const SizedBox(height: 30),
                    ErrorMessage(fadeAnimation: _animations.fadeIn),
                  ],
                ),
              ),
              const Spacer(),
              Padding(
                ...
                child: Consumer<ConnectionViewModel>(
                  builder: (context, viewModel, _) => RetryButton(
                    slideAnimation: _animations.slide,
                    onPressed: () => viewModel.retryConnection(context),
                  ),
                ),
              ),
            ],
