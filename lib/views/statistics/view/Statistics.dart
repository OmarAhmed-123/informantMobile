      ...
      body: BlocConsumer<ApiCubit, ApiState>(
        listener: (context, state) {
          state.when(
            ...
            success: (data) {
              setState(() {
                _ads = List<Map<String, dynamic>>.from(data);
              });
            },
            error: (error) {
...
import '../animations/profit_animations.dart';
...
class ProfitViewState extends State<ProfitView>
    with SingleTickerProviderStateMixin {
  late ProfitAnimations _animations;
  late ProfitViewModel _viewModel;

  @override
  void initState() {
    super.initState();
    _animations = ProfitAnimations(
      controller: AnimationController(
        duration: const Duration(milliseconds: 1500),
        vsync: this,
      ),
    );
    _viewModel = ProfitViewModel();
          ...
          builder: (context, viewModel, child) {
            ...
            return SingleChildScrollView(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  RevenueCard(slideAnimation: _animations.slideAnimation),
                  const RevenueChart(),
                  Padding(
                    padding: const EdgeInsets.all(16),
                    child: Text(
                  ...
                  AdsList(fadeAnimation: _animations.fadeAnimation),
                ],
              ),
            );
          },
