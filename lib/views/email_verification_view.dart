                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      Response isRegistered = await HttpRequest.get("user/sendotp");
                      // bool isRegistered = await authViewModel.checkEmailRegistration(_emailController.text);
                      if (isRegistered.statusCode == 200) {
                        Navigator.pushNamed(context, '/otp_verification');
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Email is not registered. Please sign up.')),
                        );
                      }
                    }
                  },
