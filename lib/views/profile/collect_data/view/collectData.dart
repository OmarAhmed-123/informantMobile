...
import '../animations/collect_animations.dart';
import '../view_model/collect_view_model.dart';
import 'widgets/collect_app_bar.dart';
import 'widgets/collect_avatar.dart';
import 'widgets/collect_form.dart';
import 'widgets/collect_submit_button.dart';

class CollectDataView extends StatefulWidget {
  const CollectDataView({Key? key}) : super(key: key);

  @override
  CollectDataViewState createState() => CollectDataViewState();
}

class CollectDataViewState extends State<CollectDataView>
    with TickerProviderStateMixin {
  late final CollectAnimations _animations;

  @override
  void initState() {
    super.initState();
    _animations = CollectAnimations(vsync: this);
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (_) => CollectViewModel(),
      child: Consumer<CollectViewModel>(
        builder: (context, viewModel, _) {
          return Scaffold(
            appBar: CollectAppBar(
                ...
                child: Padding(
                  ...
                  child: ListView(
                    children: [
                      CollectAvatar(
                        viewModel: viewModel,
                        scaleAnimation: _animations.avatarScaleAnimation,
                      ),
                      const SizedBox(height: 30),
                      CollectForm(
                        viewModel: viewModel,
                        fieldAnimations: _animations.fieldAnimations,
                      ),
                      const SizedBox(height: 30),
                      CollectSubmitButton(
                        viewModel: viewModel,
